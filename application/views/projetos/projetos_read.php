...
<div class="col-lg-12 col-md-12 col-sm-12">
	<div class="card">
		<div class="card-title">
			<h4>
				<i class="fa fa-eye"></i>
				<?= $this->lang->line('app_view').' '.ucfirst($this->lang->line('projects')); ?>
			</h4>
		</div>
		<div class="card-body">
			...
			<div class="tabs">
				<ul class="nav nav-tabs customtab2" role="tablist">
					<li class="nav-item"> <a class="nav-link active show" data-toggle="tab" href="#info" role="tab" aria-selected="true"><span class="hidden-sm-up"><i class="ti-menu-alt"></i></span> <span class="hidden-xs-down"><?= ucfirst($this->lang->line('project')); ?></span> </a> </li>
					<li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#tasks" role="tab" aria-selected="false"><span class="hidden-sm-up"><i class="ti-check-box"></i></span> <span class="hidden-xs-down"><?= ucfirst($this->lang->line('tasks')); ?></span> </a> </li>
				</ul>
				<!-- Tab panes -->
						...
						<form id="form_delete" method="post">
							<table style="width:100%" id="table" class="table table-bordered table-striped">
								<thead>
									<tr>
										<th>
											<?php if ($this->permission->check($this->session->userdata('permissao'), 'dTarefas')) { ?>
			...
			<a href="<?= site_url('projetos/create') ?>" class="btn btn-success">
				<i class="fa fa-plus"></i>
				<?= $this->lang->line('app_create'); ?>
			</a>
			<?php } ?>
			<?php if ($this->permission->check($this->session->userdata('permissao'), 'eProjetos')) { ?>
			<a href="<?= site_url('projetos/update/'.$IdProjeto) ?>" class="btn btn-info">
...
<script type="text/javascript">  
	$(document).ready(function () {
		...
		var datatable = $('#table').DataTable({
			...
			"order": [],
			"ajax": {
				url: "<?= site_url('tarefas/datatable/'.$IdProjeto); ?>",
				type: "POST"
			},
			"columnDefs": [
				{
