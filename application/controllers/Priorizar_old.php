...
class Priorizar extends CI_Controller
{
    ...
    public function index()
    {
		...
		if (!$this->permission->check($this->session->userdata('permissao'),
                'fConselho'))
            {
                if (!$this->permission->check($this->session->userdata('permissao'),
                    'fGerset'))
    ...
    public function datatable()
    {
        ...
        foreach ($result_data as $row) {
			...
			$line[] = number_format($row->autorizado, 2, ',', '.');
			$line[] = $row->gerset;
			if (($this->permission->check($this->session->userdata('permissao'), 'fConselho')) || ($this->permission->check($this->session->userdata('permissao'), 'fGerset')))
        	{
        ...
        $output = array(
            ...
            'recordsTotal' => $this->Priorizar_model->get_all_data(),
            'recordsFiltered' => $this->Priorizar_model->get_filtered_data(),
            'data' => $data,
        );
	...
	public function setOrdem()
    {
        ...
        $this->load->model('Priorizar_model');
		if(
        $this->Priorizar_model->setOrdemPrioridade($tar_id_item) &&
        $this->distribuirRecursos($tar_id_proj)
		) {
 			$this->session->set_flashdata('success', $this->lang->line('app_priority_success'));
		  }
    }
	
	public function distribuirRecursos($id)
    {
        
		$results = $this->Priorizar_model->getTarefasByProjeto($id);
        $result = $this->Priorizar_model->getAutorizadoByProjeto($id);
        $margem = $this->Priorizar_model->getMargemReserva()->margem_reserva;
        $valor_restante = $result->valor_autorizado;
        $valor_reserva = ($result->valor_autorizado * $margem) / 100;
        ...
        foreach ($results as $r)
        {
            ...
            if ($valor_restante > 0)
            {
                if ($valor_orcado >= $valor_restante)
                {
                    ...
                    $this->Priorizar_model->setValorAutorizado($r->IdTarefa, $valor_autorizado, 2);
                    $valor_restante = 0;
                    if ($valor_restante == 0 && $valor_orcado > $valor_autorizado && $valor_restante2 >
                        0)
                    {
                        $valor_excedente = $valor_orcado - $valor_autorizado;
                        if ($valor_excedente >= $valor_restante2)
                        {
                            $valor_autorizado = $valor_autorizado + $valor_restante2;
                            $this->Priorizar_model->setValorAutorizado($r->IdTarefa, $valor_autorizado, 3);
                            $valor_restante2 = 0;
                        } else
                            if ($valor_orcado < $valor_restante2)
                            {
                                $valor_autorizado = $valor_autorizado + $valor_excedente;
                                $this->Priorizar_model->setValorAutorizado($r->IdTarefa, $valor_autorizado, 3);
                                $valor_restante2 = $valor_restante2 - $valor_excedente;
                            }
                    }
                } else
                    if ($valor_orcado < $valor_restante)
                    {
                        $valor_autorizado = $valor_orcado;
                        $this->Priorizar_model->setValorAutorizado($r->IdTarefa, $valor_autorizado, 2);
                        $valor_restante = $valor_restante - $valor_autorizado;
                    }
            } else
                if ($valor_restante2 > 0)
                {
                    if ($valor_orcado >= $valor_restante2)
                    {
                        $valor_autorizado = $valor_restante2;
                        $this->Priorizar_model->setValorAutorizado($r->IdTarefa, $valor_autorizado, 1);
                        $valor_restante2 = $valor_restante2 - $valor_autorizado;
                    } else
                        if ($valor_orcado < $valor_restante2)
                        {
                            $valor_autorizado = $valor_orcado;
                ...
                {
                    $valor_autorizado = 0;
                    $this->Priorizar_model->setValorAutorizado($r->IdTarefa, $valor_autorizado, 0);
                }
        }
		return true;
    }	
	...
	public function exportar($id)
    {
        ...
        $spreadsheet->getProperties()->setCreator($this->session->userdata('nome'))->
            setLastModifiedBy($this->session->userdata('nome'))->setTitle('SPO - ' . $this->
            session->userdata('anofiscal') . ' Priorizada')->setSubject('Planilha SPO')->
            setDescription('Planilha SPO Priorizada.')->setKeywords('SPO')->setCategory('Arquivo SPO');
        ...
        $spreadsheet->getActiveSheet()->getStyle('D2:F255')->getAlignment()->
            setWrapText(true);
        $spreadsheet->getActiveSheet()->getStyle('G2:G255')->getNumberFormat()->
            setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);

        // set the names of header cells
        $spreadsheet->setActiveSheetIndex(0)->setCellValue("A1", 'Prioridade')->
            setCellValue("B1", 'Nº SPO')->setCellValue("C1", 'Setor')->setCellValue("D1",
            'Título')->setCellValue("E1", 'Descrição')->setCellValue("F1", 'Justificativa')->
            setCellValue("G1", 'Valor Estimado (R$)');
        ...
        if (!$tarefas)
        {
            ...
            $spreadsheet->setActiveSheetIndex(0)->setCellValue("A2",
                'Não há tarefas cadastradas');
        } else
        {
            foreach ($tarefas as $t)
            {
                $spreadsheet->setActiveSheetIndex(0)->setCellValue("A$x", $t->prioridade)->
                    setCellValue("B$x", $t->spo_id)->setCellValue("C$x", $t->sigla)->setCellValue("D$x",
                    $t->titulo)->setCellValue("E$x", $t->descricao)->setCellValue("F$x", $t->
                    justificativa)->setCellValue("G$x", $t->valor_previsto);
                $x++;
            }
            $y = $x;
            $x = $x - 1;
            ...
            $spreadsheet->getActiveSheet()->getStyle("A$y")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::
                HORIZONTAL_RIGHT);
            $spreadsheet->getActiveSheet()->getStyle('A' . $y . ':G' . $y)->getFont()->
                setBold(true);
	...
	public function addAmount($id)
    {
        ...
        $data = array(
            ...
            'action' => site_url('priorizar/addamount_action'),
			'IdProjeto' => set_value('IdProjeto', $id),
			'valor' => set_value('valor'),
            'data_autorizacao' => set_value('data_autorizacao'),
			'ano_autorizacao' => $this->session->userdata('anofiscal'),
        );
	...
	function addamount_action()
    {
		...
		if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'projeto_id' => $this->input->post('IdProjeto'),
            ...
            {
                $this->distribuirRecursos($this->input->post('IdProjeto'));
	...
	public function delAmount($id)
    {
        ...
        $data = array(
            'button' => '<i class="fa fa-minus"></i> ' . $this->lang->line('app_remove'),
            'action' => site_url('priorizar/delamount_action'),
			'IdProjeto' => set_value('IdProjeto', $id),
			'valor' => set_value('valor'),
            'data_autorizacao' => set_value('data_autorizacao'),
			'ano_autorizacao' => $this->session->userdata('anofiscal'),
